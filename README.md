...
`sample-01`–`sample-04` 保留给未来可核验的原始发布样张。以上八张包括四张 16:9 横版左右样张和四张 3:4 竖版上下样张，均由 Panel 085 依据自己的原始提示词独立生成，不借用其他编号作品。样张使用英文智能配字。

<!-- xxd-human-intro:start -->
## 适用场景与解决的问题

完整复制照片会塞满信息，纯 3D 渲染又会失去材料温度；真正的微缩装置需要小、静、准，并且让手工痕迹成为叙事的一部分。

**Panel 085** 将照片的主体、材质关系和叙事结构缩小为下半部分约 20%–30% 的收藏级手工舞台：黏土、毛毡、手剪纸、线绳、纸板和少量哑光材料共同搭建一个一眼可认的小型装置。指纹、纤维、纸张厚度、手剪边缘、线绳弯曲与接缝都被保留，四周大量留白让材料和主体成为唯一焦点。

### 适合这些情况

- **想把复杂照片压缩成收藏级小舞台：** 只保留主体身份、材质关系和必要环境，让微缩装置承担完整叙事。
- **重视真实手工温度：** 让黏土指纹、毛毡纤维、手剪边缘、线绳弯曲和纸板接缝成为可见语言。
- **需要小主体与大留白的高级感：** 将主体控制在约 20%–30%，以哑光材料、浅底色和少量记忆色建立焦点。
...
> 用 XXD Panel 085 帮我处理这张图，先推荐最合适的构图和尺寸。
...
**关键词：** 手工微缩舞台 · 收藏级立体封面 · 黏土与毛毡 · 手剪纸与线绳 · 哑光材料 · 艺术留白

<!-- xxd-panel-benefit:start -->
## 快速判断：XXD Panel 085 适合你吗？

| 你关心的问题 | 这套风格给你的回答 |
|---|---|
| **你会得到什么** | 一幅像独立出版物封面、艺术展览海报与手工模型摄影的微缩作品 |
| **一眼可辨的特点** | 主体很小、留白很大，真实纤维、指纹、剪切边缘、接缝与柔和哑光阴影清晰可见 |
| **它如何尊重你的输入** | 保留主体身份、材质关系、空间重心和叙事结构，只把场景压缩成一个安静装置 |
...
```text
提取主体、轮廓、材质关系与叙事结构 → 将复杂场景压缩为一个微缩舞台 → 以黏土、毛毡、纸、线绳和纸板搭建 → 保留指纹、纤维、厚度、剪切边缘与接缝 → 将主体控制在 20%–30% 并留出大面积空白 → 从原图提取少数鲜活色并适度提纯 → 用小型贴纸、压印标签或手切纸片承载少量文字
```

## 成品中最容易识别的风格特点

- 主体约占下半部分 20%–30%，居中偏下或按原图重心轻微偏移，绝不填满场景。
- 材料保持哑光、柔和、真实：指纹、毛毡纤维、纸张厚度、手剪边缘、线绳弯曲和接缝都可见。
- 允许轻微不对称、歪斜与制作误差，但拒绝光滑 CG、塑料反光和纯 3D 模型感。
- 配色从上方照片提取少数生命力颜色，背景为极浅接近白色，避免灰脏、暗沉、颜色过多和廉价糖果感。
- 文字只加入真正有意义的少量标题、短句或注释，可作为贴纸、压印标签或手切纸片。

原始审美约束与拒绝项只存在于[原始提示词](references/original-prompt/zh-CN.md)；Skill 与运行适配器只处理本次交付变量。 [Skill 工作流](SKILL.md) · [英文运行适配器](references/xxd-panel-085-prompt.en.md)
...
直接传入一个图片目录时，这个士兵 Skill 会自动批量处理目录中的图片，并让每张图片都使用 Panel 085 的同一套原始审美。默认递归扫描常见位图，先报告数量，再统一确认一次模式、尺寸、文字和语言；每张图片独立生成，互不借用内容或文案。整批仍然只创建一个扁平任务目录，单张失败会被记录，但不会静默跳过后续图片。
...
/xxd-panel-085 photo.jpg --mode top-bottom --size auto --text prompt --locale zh-CN
...
# 模型依照原始提示词决定文字，只限定语言
/xxd-panel-085 photo.jpg --mode design-only --size 3:4 --text prompt --locale en-GB

# 使用准确文案，不改写、不翻译、不补标题
/xxd-panel-085 photo.jpg --mode design-only --size 3:4 --copy "让风经过这里" --locale zh-CN

# 两种模式 × 三种尺寸，共生成六张独立完整构图
/xxd-panel-085 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale zh-CN

# 整个目录统一使用 Panel 085；公共设置只解析一次
/xxd-panel-085 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale zh-CN
...
Claude Code 用户可把同一文件夹链接到 `~/.claude/skills/xxd-panel-085`. 安装后请重启 Agent 会话。
...
完整规格: [Skill 工作流](SKILL.md) · [原始风格档案](references/original-prompt/zh-CN.md) · [英文运行适配器](references/xxd-panel-085-prompt.en.md) · [中文运行适配器](references/xxd-panel-085-prompt.zh-CN.md)

<!-- xxd-panel-catalog:start -->
## XXD Panel 全系列项目

85 个 Panel 各自保留独立的原始提示词与审美逻辑。下面连续列出 001–085 的全部项目地址和核心风格特点；当前项目以粗体标出。
...
| **士兵级** | `xxd-panel-NNN`（当前 001–085） | 每个编号只执行自己独立的原始提示词与审美，把将军派发的单个任务完成为成品。 |
