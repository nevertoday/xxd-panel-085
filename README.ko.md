<p align="center"><img src="./assets/banner.svg" alt="XXD Panel 085 프로젝트 배너" width="1200"></p>
...
`sample-01`–`sample-04`는 검증 가능한 원본 공개 예시를 위해 남겨 둡니다. 위 여덟 작품은 16:9 가로형 좌우 구성 네 장과 3:4 세로형 상하 구성 네 장입니다. 모두 Panel 085이 자체 원본 프롬프트에 따라 독립적으로 생성했으며 다른 번호의 작품을 재사용하지 않았습니다. 예시 문구는 영어로 자동 생성했습니다.
...
```text
정체성·자세·재료 관계·서사 구조 추출 → 필수 공간 단서 외 삭제 → 점토·펠트·손절 종이·실·판지로 한 무대 재구축 → 피사체를 약 20%–30%로 제한 → 지문·섬유·절단면·굽음·이음매 유지 → 백색에 가까운 바탕과 한두 기억색 사용 → 필요하면 작은 물리 라벨 통합
```

## 완성작의 특징

- 미니어처는 아래쪽 절반의 작은 영역만 차지하고 독립 전시된 무대로 읽힙니다.
- 점토 흔적, 펠트 섬유, 종이 두께, 손절 가장자리, 실 굽음, 판지 결합이 촉각적으로 보입니다.
- 약간의 비대칭과 제작 오차는 허용하지만 광택 CG와 플라스틱 반사는 피합니다.
- 옅은 바탕에서 원본의 한두 색만 기억의 닻으로 사용합니다.
- 글자는 작은 스티커, 압인 라벨, 종이 조각, 절제된 기록 주석으로 나타납니다.

미적 제약은 [원본 프롬프트](references/original-prompt/zh-CN.md)에만 있습니다. Skill과 실행 어댑터는 납품 변수만 처리합니다. [Skill](SKILL.md) · [영문 실행 어댑터](references/xxd-panel-085-prompt.en.md)
...
| **[xxd-panel-085](https://github.com/nevertoday/xxd-panel-085)** | 수공 미니어처 무대 · 소장형 입체 표지 · 점토와 펠트 · 손으로 자른 종이와 끈 · 무광 촉감 · 예술적 여백 |
...
| **병사급** | `xxd-panel-NNN` (현재 001–085) | 각 번호가 고유한 원본 프롬프트와 미학만 실행해 장군이 배정한 하나의 작업을 완성합니다. |
