...
<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-05.png" alt="XXD Panel 085 Sample 5"></td>
    <td width="50%"><img src="./assets/examples/sample-06.png" alt="XXD Panel 085 Sample 6"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-07.png" alt="XXD Panel 085 Sample 7"></td>
    <td width="50%"><img src="./assets/examples/sample-08.png" alt="XXD Panel 085 Sample 8"></td>
  </tr>
</table>
...
<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-09.png" alt="XXD Panel 085 additional top-bottom sample 1"></td>
    <td width="50%"><img src="./assets/examples/sample-10.png" alt="XXD Panel 085 additional top-bottom sample 2"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-11.png" alt="XXD Panel 085 additional top-bottom sample 3"></td>
    <td width="50%"><img src="./assets/examples/sample-12.png" alt="XXD Panel 085 additional top-bottom sample 4"></td>
  </tr>
</table>
...
위 여덟 작품은 16:9 가로형 좌우 구성 네 장과 3:4 세로형 상하 구성 네 장입니다. 모두 Panel 085이 자체 원본 프롬프트에 따라 독립적으로 생성했으며 다른 번호의 작품을 재사용하지 않았습니다. 예시 문구는 영어로 자동 생성했습니다.

<!-- xxd-human-intro:start -->
## 이 스타일이 해결하는 문제

사진을 완전히 복제하면 화면이 정보로 가득 차고 순수 3D 렌더링은 재료 온기를 잃습니다. 설득력 있는 미니어처는 작고 조용하고 정확해야 하며 제작 흔적이 서사의 일부가 되어야 합니다.

**Panel 085**는 피사체, 재료 관계, 이야기를 아래쪽 절반의 약 20%–30%만 차지하는 소장급 수제 무대로 줄입니다. 점토, 펠트, 손으로 자른 종이, 실, 판지, 무광 재료가 하나의 식별 가능한 설치물을 만들고 지문, 섬유, 절단면, 굽음, 이음매가 넓은 여백 속에 남습니다.

### 이런 경우에 적합합니다

- 복잡한 사진을 정체성과 필요한 환경만 가진 소장급 작은 무대로 압축하고 싶을 때
- 지문, 펠트 섬유, 손으로 자른 가장자리, 굽은 실, 판지 이음매를 시각 언어로 삼고 싶을 때
- 무광 재료, 옅은 바탕, 한두 기억색으로 작은 피사체와 큰 여백의 고급감을 만들고 싶을 때
- 상하·좌우·디자인 전용·여러 비율·벽지·폴더 일괄 처리

### 빠른 시작

> 이 이미지를 XXD Panel 085로 처리하고 가장 알맞은 구도와 크기를 먼저 추천해 주세요.
...
| 확인할 점 | 이 스타일의 답 |
|---|---|
| **결과물** | 독립 출판 표지, 전시 포스터, 수제 모델 사진을 결합한 미니어처 작품 |
| **대표 특징** | 아주 작은 피사체, 넓은 여백, 섬유, 지문, 절단면, 이음매, 부드러운 무광 그림자 |
| **입력 존중 방식** | 정체성, 재료 관계, 공간 무게중심, 서사 구조를 지키면서 한 조용한 설치물로 압축함 |
...
```bash
git clone https://github.com/nevertoday/xxd-panel-085.git
mkdir -p ~/.codex/skills
ln -s "$(pwd)/xxd-panel-085" ~/.codex/skills/xxd-panel-085
```

`npx skills`로도 바로 설치할 수 있습니다:

```bash
npx skills add https://github.com/nevertoday/xxd-panel-085 --skill xxd-panel-085
```
...
Claude Code 사용자는 같은 폴더를 다음 위치에 연결할 수 있습니다: `~/.claude/skills/xxd-panel-085`. 설치 후 에이전트 세션을 다시 시작하세요.
...
전체 사양: [Skill 워크플로](SKILL.md) · [원본 스타일 자료](references/original-prompt/zh-CN.md) · [영문 런타임 어댑터](references/xxd-panel-085-prompt.en.md) · [중문 런타임 어댑터](references/xxd-panel-085-prompt.zh-CN.md)
...
**특징:** 수공 미니어처 무대 · 소장형 입체 표지 · 점토와 펠트 · 손으로 자른 종이와 끈 · 무광 촉감 · 예술적 여백
...
이미지 폴더를 직접 전달하면 이 병사 Skill이 같은 Panel 085의 원본 미학으로 일괄 처리합니다. 일반 래스터 형식을 기본적으로 재귀 탐색하고 수량을 먼저 알린 뒤 모드·크기·텍스트·언어를 한 번만 확인합니다. 각 이미지는 내용이나 문구가 섞이지 않도록 독립 생성하며 전체 배치는 하나의 평면 작업 디렉터리만 사용합니다. 개별 실패는 기록하고 이후 이미지를 조용히 누락하지 않습니다.
...
```text
/xxd-panel-085 photo.jpg --mode top-bottom,design-only --size auto,3:4,9:16 --text prompt --locale ja-JP
```
...
```text
/xxd-panel-085 photo.jpg --prefs last
/xxd-panel-085 photo.jpg --prefs edit
/xxd-panel-085 photo.jpg --prefs new
```
...
/xxd-panel-085 photo.jpg --mode top-bottom --size auto --text prompt --locale ko-KR

# 좌우 비교: 3:2 가로형, 글자 없음
/xxd-panel-085 photo.jpg --mode left-right --size 3:2 --text none

# 디자인 전용: 9:16 휴대폰 세로형, 한국어 문구
/xxd-panel-085 photo.jpg --mode design-only --size 9:16 --text prompt --locale ko-KR

# 연결형 4종 배경화면: 기준 이미지 후 기기별 재구성
/xxd-panel-085 photo.jpg --mode wallpaper-pack --wallpaper linked --text none

# 독립형 배경화면: 기기별 정확한 해상도 지정
/xxd-panel-085 photo.jpg --mode wallpaper-pack --wallpaper independent --wallpaper-size phone=1440x3200,ipad=2048x2732,desktop=3840x2160,watch=1024x1024 --text none

# 원본 이미지 비율 따르기
/xxd-panel-085 photo.jpg --mode design-only --size source --text none

# 정사각형·세로·가로 완성 구도를 각각 생성
/xxd-panel-085 photo.jpg --mode design-only --size 1:1,3:4,16:9 --text none

# 사용자 비율과 정확한 픽셀을 함께 지정
/xxd-panel-085 photo.jpg --mode design-only --size 11:14,2160x3840,3840x2160 --text none

# 원본 프롬프트가 문구를 만들고 언어만 제한
/xxd-panel-085 photo.jpg --mode design-only --size 3:4 --text prompt --locale ko-KR

# 정확한 문구를 재작성·번역·제목 추가 없이 사용
/xxd-panel-085 photo.jpg --mode design-only --size 3:4 --copy "빛을 이곳에 남겨 두기" --locale ko-KR

# 2개 모드 × 3개 크기로 독립적인 완성 구도 6개 생성
/xxd-panel-085 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale ko-KR

# 폴더 전체에 Panel 085 적용, 공통 설정은 한 번만 확정
/xxd-panel-085 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale ko-KR

# 반복 매개변수도 누적하며 지정 루트 아래로 출력
/xxd-panel-085 photo.jpg --mode top-bottom --mode left-right --size 3:4 --size 16:9 --text none --out ./deliveries
```
...
현재 XXD Panel 시리즈는 001–112까지 업데이트되었으며, 각 Panel은 독립된 원본 프롬프트와 미적 논리를 유지합니다. 아래 표는 이 프로젝트 공개 시점의 과거 목록으로 001–085을 연속해서 나열하며, 현재 프로젝트는 굵게 표시합니다.
