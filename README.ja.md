...
上の8点は、16:9の横長・左右構成4点と、3:4の縦長・上下構成4点です。すべて Panel 085 が自身の原文プロンプトに基づいて個別に生成したもので、別番号の作品は流用していません。作例のコピーは英語で自動生成しています。

<!-- xxd-human-intro:start -->
## このスタイルが解決すること

写真の完全コピーは情報で紙面を埋め、純粋な3Dレンダリングは素材の温度を失います。説得力のあるミニチュアには、小ささ、静けさ、正確さと、物語を担う制作痕跡が必要です。

**Panel 085** は主体、素材関係、物語を下半分の約20%〜30%だけを占めるコレクタブルな手作り舞台へ縮小します。粘土、フェルト、手切り紙、糸、厚紙、マット素材で一つの識別可能な装置を作り、指紋、繊維、切り口、曲がり、継ぎ目を大きな余白の中に残します。

### 向いている用途

- 複雑な写真を、同一性と必要環境だけを持つコレクタブルな小舞台へ圧縮したい場合
- 指紋、フェルト繊維、手切りの縁、曲がった糸、厚紙の継ぎ目を視覚言語にしたい場合
- マット素材、淡い地色、一、二色の記憶色で小主体・大余白の上質感を作りたい場合
- 上下・左右・デザインのみ・複数比率・壁紙・フォルダ一括処理

### すぐ試す

> この画像を XXD Panel 085 で処理し、最適な構図とサイズを先に提案してください。
...
| 確認したいこと | このスタイルの答え |
|---|---|
| **得られるもの** | 独立出版表紙、展覧会ポスター、手作り模型写真を合わせたミニチュア作品 |
| **識別しやすい特徴** | 非常に小さな主体、大きな余白、繊維、指紋、切り口、継ぎ目、柔らかなマット影 |
| **入力の尊重** | 同一性、素材関係、空間重心、物語構造を保ちながら一つの静かな装置へ圧縮する |
...
Claude Code では同じフォルダを次へリンクできます： `~/.claude/skills/xxd-panel-085`. インストール後に Agent セッションを再起動してください。
...
完全仕様: [Skill ワークフロー](SKILL.md) · [原始スタイル資料](references/original-prompt/zh-CN.md) · [英語ランタイムアダプター](references/xxd-panel-085-prompt.en.md) · [中国語ランタイムアダプター](references/xxd-panel-085-prompt.zh-CN.md)
...
**特徴：** 手作り微縮舞台 · コレクタブル立体表紙 · 粘土とフェルト · 切り紙と糸 · マットな触感 · 芸術的余白
...
画像フォルダーを直接渡すと、この兵士 Skill が同じ Panel 085 の原文美学で一括処理します。一般的なラスター形式を既定で再帰検索し、件数を報告してから、モード・サイズ・文字・言語を一度だけ確認します。各画像は内容や文言を混ぜず独立生成し、バッチ全体は一つのフラットなタスクフォルダーへ保存します。個別の失敗は記録し、後続画像を黙って省略しません。
...
/xxd-panel-085 photo.jpg --mode top-bottom --size auto --text prompt --locale ja-JP

# 左右比較：3:2 横長、文字なし
/xxd-panel-085 photo.jpg --mode left-right --size 3:2 --text none

# デザインのみ：9:16 スマートフォン縦長、日本語文字
/xxd-panel-085 photo.jpg --mode design-only --size 9:16 --text prompt --locale ja-JP
...
/xxd-panel-085 photo.jpg --mode design-only --size 3:4 --text prompt --locale ja-JP

# 正確な文言を、書き換え・翻訳・タイトル追加なしで使用
/xxd-panel-085 photo.jpg --mode design-only --size 3:4 --copy "光をここに残す" --locale ja-JP

# 2モード × 3サイズで独立した完成構図を6枚生成
/xxd-panel-085 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale ja-JP

# フォルダー全体へ Panel 085 を適用し、共通設定は一度だけ解決
/xxd-panel-085 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale ja-JP
...
XXD Panel の現行シリーズは 001〜112 まで更新され、各 Panel は独立した原文プロンプトと美的ロジックを保持しています。以下は本プロジェクト公開時点の履歴カタログで、001〜085 を連続して掲載し、現在のプロジェクトを太字で示します。
