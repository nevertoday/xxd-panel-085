<p align="center"><img src="./assets/banner.svg" alt="XXD Panel 085 プロジェクトバナー" width="1200"></p>
...
`sample-01`–`sample-04` は、検証可能なオリジナル公開作例のために確保しています。上の8点は、16:9の横長・左右構成4点と、3:4の縦長・上下構成4点です。すべて Panel 085 が自身の原文プロンプトに基づいて個別に生成したもので、別番号の作品は流用していません。作例のコピーは英語で自動生成しています。
...
```text
同一性・姿勢・素材関係・物語構造を抽出 → 必要な空間手掛かり以外を削除 → 粘土・フェルト・手切り紙・糸・厚紙で一つの舞台を再構築 → 主体を約20%〜30%に抑える → 指紋・繊維・切り口・曲がり・継ぎ目を残す → 白に近い地色と一、二色の記憶色を使う → 必要なら小さな物理ラベルを統合する
```

## 完成作の特徴

- ミニチュアは下半分の小領域だけを占め、独立展示された舞台として読める。
- 粘土跡、フェルト繊維、紙厚、手切りの縁、糸の曲がり、厚紙接合が触覚的に見える。
- わずかな非対称と制作誤差を許し、光沢CGとプラスチック反射は避ける。
- 淡い地色の中で元写真由来の一、二色だけを記憶の錨にする。
- 文字は小さなステッカー、型押しラベル、紙片、控えめな記録注記として現れる。

美的制約は[原文プロンプト](references/original-prompt/zh-CN.md)だけにあります。Skill と実行アダプターは納品変数のみを扱います。 [Skill](SKILL.md) · [英語実行アダプター](references/xxd-panel-085-prompt.en.md)
...
| **[xxd-panel-085](https://github.com/nevertoday/xxd-panel-085)** | 手作り微縮舞台 · コレクタブル立体表紙 · 粘土とフェルト · 切り紙と糸 · マットな触感 · 芸術的余白 |
...
| **兵士級** | `xxd-panel-NNN`（現在 001–085） | 各番号が固有の原文プロンプトと美学だけを実行し、将軍から渡された一つの仕事を完成させます。 |
